...
process_state<-function(state)
{
  ...
  if (file.exists(file_input)){
    ...
    if (nrow(df_golden)>0 & length(unique(df_golden$positive))>1) {
      ...
      train = df_golden
      test = df_golden
      res.svc.lin = svm(train$positive~.,train %>% dplyr::select(-positive),
                        kernel="linear",scale=F,cost=1, tolerance = 0.005)
      
      
      file_output = paste0(output_path, file_short_rds)
      #file_output = paste0("/Volumes/Simo/models_svm/", file_short_rds)
      saveRDS(res.svc.lin,file_output)
    }
