  ...
  if (file.exists(file_input)){
    ...
    if (nrow(df_golden)>0 & length(unique(df_golden$positive))>1) {
      ...
      res.glm = glm(train$positive~., 
                    data=train %>% dplyr::select(-positive),
                    family=binomial
      )
      
      
      file_output = paste0(output_path, file_short_rds)
      #file_output = paste0("/Volumes/Simo/models_nn/", file_short_rds)
      saveRDS(res.glm,file_output)
    }
